...
import contractArtifact from './YondoMondo.json';
import whitelist from './whitelist.json';
import PreSaleDialog from './dialog';
  ...
  methods: {
    async handleConnectWallet() {
      try {
        ...
        if (! whitelist.map(x => x.trim().toUpperCase()).includes(this.currentAccount.trim().toUpperCase())) {
          throw new Error('Apologies, this wallet address is not on our whitelist')
        }
    ...
    async handleMintYondo() {
      try {
        ...
        const saleIsActive = await contract.preSaleIsActive()

        if (! saleIsActive) {
          throw new Error('Presale is not live yet')
        }

        const gasLimit = Math.floor((await contract.estimateGas.presaleMint(Number(this.amount), {from: this.currentAccount, value: ethers.utils.parseEther(mintPrice)})) * 1.25);
        ...
        const batchPurchaseResult = await contract.presaleMint(Number(this.amount), purchaseOptions);
        this.$toast.open({
          message:
